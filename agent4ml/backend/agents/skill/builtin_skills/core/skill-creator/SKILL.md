...
## Agent4ML skill format

A Agent4ML skill lives in a directory containing `SKILL.md`:

```
skills/<category>/<skill-name>/SKILL.md      # user skills
agent4ml/backend/agents/skill/builtin_skills/<category>/<skill-name>/SKILL.md   # builtin
```
...
2. **Tools referenced in prose must be native Agent4ML tools** (listed above) or
...
- Naming shell utilities instead of Agent4ML tools → model hallucinates calls
