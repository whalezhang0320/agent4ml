...
This skill helps you discover skills available in Agent4ML's builtin library
...
Agent4ML ships a builtin library at:

```
agent4ml/backend/agents/skill/builtin_skills/
...
```bash
list_dir("agent4ml/backend/agents/skill/builtin_skills/")
```
...
```bash
bash("grep -rl '<keyword>' agent4ml/backend/agents/skill/builtin_skills/ --include='SKILL.md'")
```

Or read a category directory and inspect each `SKILL.md` frontmatter:

```bash
list_dir("agent4ml/backend/agents/skill/builtin_skills/research/")
```

Then read a candidate:

```bash
read_file("agent4ml/backend/agents/skill/builtin_skills/research/deep-research/SKILL.md")
```
