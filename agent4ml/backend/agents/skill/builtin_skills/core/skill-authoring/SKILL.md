...
# Authoring Agent4ML Skills

## Overview

A SKILL.md can live in two places:

1. **Builtin (in-repo):** `agent4ml/backend/agents/skill/builtin_skills/<category>/<name>/SKILL.md`
...
- You're committing a reusable workflow that should ship with Agent4ML
- You're editing an existing skill under `builtin_skills/`

## Required Frontmatter

Source of truth: `agent4ml/backend/agents/skill/parser.py::parse_skill_file`.
Hard requirements:
...
allowed-tools:                    # Agent4ML tools this skill may invoke
  - bash
  - read_file
  - write_file
  - list_dir
...
Tools referenced in SKILL.md prose must be native Agent4ML tools (listed in
...
1. **Survey peers** in the target category:
   ```
   list_dir("agent4ml/backend/agents/skill/builtin_skills/<category>/")
   ```
...
4. **Naming shell utilities.** `grep`/`cat`/`sed`/`find` → use Agent4ML tool
...
- [ ] No shell utility names in prose (use Agent4ML tool names)
