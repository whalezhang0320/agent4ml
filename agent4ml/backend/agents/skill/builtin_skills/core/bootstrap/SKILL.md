...
the user is and what they need, then generate a tight `.agent4ml/profile.md`
that defines the agent's working relationship with this user.

> **Agent4ML note:** The original deer-flow skill generates a `SOUL.md` via a
> `setup_agent` tool. Agent4ML has no `setup_agent` tool or SOUL.md concept.
> This version generates `.agent4ml/profile.md` (a user profile) via `write_file`.
...
4. Save with `write_file` to `.agent4ml/profile.md`:

```
write_file(".agent4ml/profile.md", "<full profile content>")
```

5. After saving, confirm: "✅ [Name]'s profile is saved at `.agent4ml/profile.md`."
...
- **Not saving** — always `write_file` to `.agent4ml/profile.md`; don't just
  print to chat
