...
> **Agent4ML note:** The original skill dispatches a fresh subagent per task via
> `delegate_task`. Agent4ML has no subagents, so this version executes tasks
...
```
read_file(".agent4ml/plans/feature-plan.md")
```
