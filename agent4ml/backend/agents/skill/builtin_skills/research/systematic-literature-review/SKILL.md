...
> Agent4ML has neither, so this version uses `bash` with `curl` to the arXiv API
...
> **Agent4ML note:** The original skill delegates extraction to parallel
> subagents. Agent4ML has no subagents, so extract sequentially in your own
...
Save the full report to `.agent4ml/outputs/slr-<topic-slug>-<YYYYMMDD>.md` via
